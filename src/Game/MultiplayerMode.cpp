...
MultiplayerMode::CreateNewGame()
{
  ...
  for (auto& i : m_list_player) {
    if (!i.SetBeginScore(0)) return false;
  }
  return true;
}

bool
MultiplayerMode::DoMove(uint8_t x, uint8_t y, const std::vector<COMMON::EDirect>& directions)
{
  ...
  if (!m_list_player[m_active_user].AddPoints((int16_t)directions.size())) return false;
